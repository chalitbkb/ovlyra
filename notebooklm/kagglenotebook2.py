  ...
  "cells": [
    ...
    },
    {
      ...
      "source": [
        ...
        "\n",
        "# --- 📂 Paths & Dataset ---\n",
        "USER_KAGGLE  = 'chalitguza'\n",
        "INPUT_NOTEBOOK_NAME = f'{USER_KAGGLE}/download1'\n",
        ...
        "\n",
        "# ====================================================================\n",
        "# 🎯 CONFIG: Kaggle T4 x2 (Balanced: stability + speed + memory safety)\n",
        "# ====================================================================\n",
        "# แนวคิด:\n",
        "# - ลด OOM: max_seq_len 1536 + gradient checkpointing\n",
        "# - เพิ่มความเร็วเทรน: eval/save ไม่ถี่เกินไป, num_workers สูงขึ้นเล็กน้อย\n",
        "# - คงคุณภาพ: LoRA rank 16, lr ไม่สูงเกินไป, warmup พอดี\n",
        "# ====================================================================\n",
        "\n",
        "config = {\n",
        "    \"training\": {\n",
    ...
    },
    {
      ...
      "source": [
        "os.chdir(REPO_DIR)\n",
        "\n",
        "# ใช้ 2 GPU บน Kaggle (ถ้ามีมากกว่า 2 ก็จำกัดที่ 2 เพื่อเสถียรภาพ)\n",
        "train_gpu_count = min(2, gpu_count)\n",
        "if train_gpu_count < 2:\n",
        "    raise RuntimeError(\"ต้องใช้ GPU >= 2 ใบสำหรับ notebook นี้\")\n",
        "\n",
        "# ลด CPU contention บน Kaggle ให้ dataloader เสถียรขึ้น\n",
        "os.environ[\"OMP_NUM_THREADS\"] = \"2\"\n",
        "os.environ[\"MKL_NUM_THREADS\"] = \"2\"\n",
        "\n",
        "subprocess.run([\n",
        "    \"fabric\", \"run\", f\"--devices={train_gpu_count}\",\n",
    ...
    },
    {
      ...
      "source": [
        ...
        "USER_TRANSCRIPT = 'สวัสดีครับ ข้อความของคุณจะถูกประมวลผลชั่วคราวเพื่อสร้างเสียงเท่านั้น โคลนเสียงได้อย่างแม่นยำราวกับต้นฉบับ และเปลี่ยนข้อความให้เป็นเสียงธรรมชาติด้วยความหน่วงต่ำ'\n",
        "\n",
        "# โหมดถอดเสียงอัตโนมัติจาก prompt wav (ลด human mismatch)\n",
        "USE_AUTO_ASR = True\n",
        "ASR_MODEL_NAME = 'small'   # tiny/base/small/medium/large-v3\n",
        "ASR_LANGUAGE = 'th'\n",
        "\n",
        "def transcribe_prompt_asr(wav_path: str) -> str:\n",
        ...
        "    if not text:\n",
        "        raise ValueError('❌ ASR ถอดเสียงไม่สำเร็จ (empty transcript)')\n",
        "    return text\n",
        "\n",
        ...
        "    if USE_AUTO_ASR:\n",
        "        PROMPT_TRANSCRIPTION = transcribe_prompt_asr(PROMPT_WAV)\n",
        "        print('🤖 ใช้ Auto-ASR สำหรับ USER_PROMPT_WAV')\n",
        "    else:\n",
        "        PROMPT_TRANSCRIPTION = USER_TRANSCRIPT\n",
        "        if (not PROMPT_TRANSCRIPTION) or ('ข้อความที่คุณพูดในไฟล์เสียง' in PROMPT_TRANSCRIPTION):\n",
        "            raise ValueError('❌ USER_TRANSCRIPT ต้องเป็นข้อความจริงที่พูดในไฟล์เสียง prompt')\n",
        "    print('✅ พบไฟล์เสียงของคุณเองที่จะใช้โคลน')\n",
        "elif os.path.exists(DEFAULT_PROMPT_WAV):\n",
        "    PROMPT_WAV = DEFAULT_PROMPT_WAV\n",
        "    if USE_AUTO_ASR:\n",
        "        PROMPT_TRANSCRIPTION = transcribe_prompt_asr(PROMPT_WAV)\n",
        "        print('🤖 ใช้ Auto-ASR สำหรับ DEFAULT_PROMPT_WAV')\n",
        "    else:\n",
        "        try:\n",
        ...
        "\n",
        "# ตรวจสอบความยาว prompt เพื่อ zero-shot clone ให้เสถียร\n",
        "import torchaudio\n",
        "prompt_wav_tmp, prompt_sr_tmp = torchaudio.load(PROMPT_WAV)\n",
        "prompt_duration_sec = prompt_wav_tmp.shape[1] / prompt_sr_tmp\n",
        "print(f'⏱️ Prompt duration: {prompt_duration_sec:.2f}s @ {prompt_sr_tmp}Hz')\n",
        "if prompt_duration_sec < 2.0 or prompt_duration_sec > 12.0:\n",
        "    print('⚠️ แนะนำให้ใช้ prompt 3-10 วินาทีเพื่อ clone เสียงให้เสถียร')"
      ]
    },
    {
      "cell_type": "code",
      ...
      "source": [
        ...
        "]\n",
        "\n",
        "# fallback profile: เพิ่มความยาว/ความหลากหลาย generation กันเคสเสียงเงียบ\n",
        "fallback_cmd = [\n",
        "    sys.executable, 'tools/serving/inference.py',\n",
        "    f'--model_checkpoint_path={export_dir}',\n",
        "    f'--audio_encoder_path={CODEC_MODEL_PATH}',\n",
        ...
        "        print(f'\\n⚠️ โปรไฟล์หลักล้มเหลว: {e}')\n",
        "        print('🔁 กำลังลอง fallback profile...')\n",
        "        subprocess.run(fallback_cmd, check=True)\n",
        "        print(f'\\n🎉 Fallback output saved to: {OUTPUT_WAV}')\n",
        "else:\n",
    ...
    },
    {
      ...
      "source": [
        ...
        "    if file_size > 0:\n",
        "        wav_out, sr_out = torchaudio.load(OUTPUT_WAV)\n",
        "        duration_out = wav_out.shape[1] / sr_out\n",
        "        rms = torch.sqrt(torch.mean(wav_out ** 2)).item()\n",
        "        peak = wav_out.abs().max().item()\n",
        "        text_chars = len(re.sub(r'\\s+', '', TEXT_TO_SPEAK))\n",
        "        expected_min = max(1.2, text_chars / 22.0)\n",
        "        expected_max = max(4.0, text_chars / 5.0)\n",
        "        print(f'📏 duration={duration_out:.2f}s | rms={rms:.6f} | peak={peak:.6f}')\n",
        "        print(f'🧮 expected duration range (heuristic): {expected_min:.2f}s - {expected_max:.2f}s')\n",
        "        if rms < 0.003 or peak < 0.02:\n",
        "            print('⚠️ เสียงอาจเงียบเกินไป (low energy). แนะนำเพิ่ม quality ของ data, เทรนนานขึ้น, หรือใช้ fallback inference profile')\n",
        "        if duration_out < expected_min * 0.75 or duration_out > expected_max * 1.35:\n",
        "            print('⚠️ ระยะเวลาเสียงอาจไม่สมดุลกับข้อความ: ตรวจ prompt transcript, จำนวน training steps, และ generation settings')\n",
